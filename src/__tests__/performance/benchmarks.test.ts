...
describe('Performance Benchmarks', () => {
    ...
    describe('Cold Start Performance', () => {
        test('cold start search completes within 4 seconds', async () => {
    ...
    describe('Warm Search Performance', () => {
        test('warm search completes within 2 seconds', async () => {
        ...
        test('average of 10 warm searches under 2 seconds', async () => {
            // Warm up
            await bingEngine.search('warmup', 5);
            ...
            console.error(`Average warm search time: ${averageDuration.toFixed(0)}ms`);
            console.error(`Min: ${Math.min(...durations)}ms, Max: ${Math.max(...durations)}ms`);
        }, { timeout: 60000 });
    });

    describe('Concurrent Search Performance', () => {
        test('handles 5 simultaneous searches', async () => {
            const startTime = Date.now();
        ...
        test('pool mode handles concurrent searches efficiently', async () => {
            const pool = new BrowserPool({ mode: 'pool', poolSize: 3, headless: true });
    ...
    describe('Engine Comparison', () => {
        test('compare performance across engines', async () => {
            const engines = [
