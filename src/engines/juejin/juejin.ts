...
export async function searchJuejin(
  ...
  try {
    while (allResults.length < limit) {
      console.error(
        `🔍 Searching Juejin with query: "${query}", cursor: ${cursor}`,
      );

      const response = await axios.get<JuejinSearchResponse>(
    ...
    console.error(
      `✅ Juejin search completed, found ${allResults.length} results`,
    );
