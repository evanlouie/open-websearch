...
async function searchBingForLinuxDo(
  query: string,
  limit: number,
...
export async function searchLinuxDo(
  ...
  console.error(
    `🔍 Searching linux.do with "${query}" using ${config.defaultSearchEngine} engine`,
  );
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(
      `❌ Linux.do search failed using ${config.defaultSearchEngine}:`,
      errorMessage,
    );
    return [];
  }
