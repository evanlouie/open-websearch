...
async function searchBingForZhiHu(
  query: string,
  limit: number,
...
export async function searchZhiHu(
  ...
  console.error(
    `🔍 Searching zhuanlan.zhihu.com with "${query}" using ${config.defaultSearchEngine} engine`,
  );
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.error(
      `❌ zhuanlan.zhihu.com search failed using ${config.defaultSearchEngine}:`,
      errorMessage,
    );
    return [];
  }
