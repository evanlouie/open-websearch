...
export class GoogleEngine extends BaseEngine {
    ...
    protected async extractResults(page: Page, limit: number): Promise<SearchResult[]> {
        ...
        try {
            ...
            const isCaptcha = await page.$('iframe[src*="recaptcha"]').catch(() => null);
            if (isCaptcha) {
                throw new Error('Google CAPTCHA detected - search blocked. This is expected for automated requests. Try using Bing, DuckDuckGo, or Brave instead.');
            }
            throw error;
        }
